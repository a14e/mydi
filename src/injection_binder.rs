...
pub struct InjectionBinder<LastType> {
    ...
    builders: Vec<(TypeId, Box<dyn Fn(&Injector) -> anyhow::Result<Box<dyn Any + 'static>>>)>,
...
impl<LastType: Clone + 'static> InjectionBinder<LastType> {
    ...
    pub fn merge<OtherLast>(mut self, mut other: InjectionBinder<OtherLast>) -> Self {
        self.static_values.extend(mem::take(&mut other.static_values));
        self.builders.extend(mem::take(&mut other.builders));
        self.requirements_graph.extend(mem::take(&mut other.requirements_graph));
    ...
    pub fn inject_fn<
        ...
        In: TupleInjectTypes,
        FN: Fn(In) -> R + 'static
    >(self, f: FN) -> InjectionBinder<R> {
        self.inject_fn_ok::<R, In, _>(
            move |tuple| { Ok(f(tuple)) }
        )
    }
    ...
    pub fn inject_fn_raw<X: Any + Clone + 'static>(mut self,
                                                   f: impl Fn(&Injector) -> anyhow::Result<X> + 'static,
                                                   dependencies_names: Vec<(TypeId, &'static str)>,
                                                   debug_line: Option<String>,
                                                   lazy: bool) -> InjectionBinder<X> {
        let func = Box::new(move |x: &Injector| -> anyhow::Result<Box<dyn Any>> {
            Ok(Box::new((f)(x)?))
        });
        let type_id = TypeId::of::<X>();
    ...
    fn verify_missing_deps(&self,
                           additional_types: &HashSet<TypeId>,
                           short_types: bool) -> anyhow::Result<()> {
        let available_types: HashSet<_> = additional_types.iter()
            .chain(self.requirements_graph.iter().map(|(id, _)| id))
            .collect();
        let missing_local_types: Vec<(TypeId, &str, Vec<&str>)> = self.requirements_graph
            .iter()
    ...
    fn traverse_dependencies_and_verify_recursion(&self,
                                                  additional_types: &HashSet<TypeId>,
                                                  short_types: bool,
                                                  mut on_resolve: impl FnMut(TypeId) -> anyhow::Result<()>,
                                                  err_message: &'static str) -> anyhow::Result<()> {
        let mut available_types: HashSet<_> = additional_types.iter()
            .cloned()
            .collect();
        ...
        let mut left_deps: HashMap<TypeId, &Vec<TypeId>> = self.requirements_graph
            .iter()
            .map(|(k, v)| (*k, v))
        ...
        loop {
            ...
            let resolved: Vec<TypeId> = left_deps.iter()
                .filter(|(_, deps)| {
                    deps.iter()
                        .all(|x| available_types.contains(x))
        ...
        let err = anyhow::anyhow!(
            "{err_message}{cycle_type_names}"
        );
        Err(err)
    }

    fn verify_duplicates(&self,
                         additional_types: &HashSet<TypeId>,
                         short_types: bool) -> anyhow::Result<()> {
        ...
        let err = anyhow::anyhow!(
            "Dependencies duplications found: {duplicates_names}"
        );
        Err(err)
    }
    ...
    pub fn verify(&self,
                  additional_types: HashSet<TypeId>,
                  short_types: bool) -> anyhow::Result<()> {
        let additional_deps: HashSet<_> = additional_types.into_iter()
    ...
    pub fn build(mut self) -> anyhow::Result<Injector> {
        let initial_known_deps: HashSet<_> = {
            self.static_values.iter().map(|(type_id, _)| *type_id).collect()
        };
        ...
        let builders_map: HashMap<_, _> =
            mem::take(&mut self.builders).into_iter().collect();
