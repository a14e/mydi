...
pub fn derive_inject(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    derive_inject_impl(ident, data, generics)
        .unwrap_or_else(Error::into_compile_error)
        .into()
}


pub(crate) fn derive_inject_impl(ident: Ident,
                                 data: Data,
                                 mut generics: Generics) -> syn::Result<TokenStream> {
    ...
    let (inject_field, fields_types): (Vec<_>, Vec<_>) = fields_with_types_and_settings.into_iter()
    ...
    for param in generics.params.iter_mut() {
        ...
        if let GenericParam::Type(type_param) = param {
            type_param.bounds.push(TypeParamBound::Trait(syn::parse_quote!(Clone)));
        }
    }
